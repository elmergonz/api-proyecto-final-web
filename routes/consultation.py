...
async def create(consult:Consultation, token:str):
    ...
    if doc:
        if await dbdata.patient_exist(consult.patientCedula, doc['email']):
...
async def get_consultation_by_patient(cedula:str, token:str):
    ...
    if doc:
        consultations, patient = await dbdata.get_consultations_by_patient(cedula, doc['email'])
...
async def modify(consult:Consultation, consultId:str, token:str):
    ...
    if doc:
        if await dbdata.consultation_exist(consultId):
            ...
            await dbdata.update_consultation_data(consult, consultId)
